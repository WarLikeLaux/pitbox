...
trap 'rm -rf "$T" "$T"-wt1 "$T"-wt2 "$T"-smoke-remote.git' EXIT
...
echo "MAIN_BRANCH=main" >> "$T/.slots/config"
# The documented flow commits .slots, and the collect guard demands a clean main checkout.
git -C "$T" add .slots
...
echo "REQUIRE_PUSH=1" >> "$T/.slots/config"
...
sed -i '/^REQUIRE_PUSH=1$/d' "$T/.slots/config"
